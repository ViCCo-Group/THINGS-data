...
from utils import xy_to_eccrad, xy_to_various_angles, call_vol2surf
...
def list_afni_outputs(sub, afnioutdir):
    ...
    return dict(ecc=pjoin(afnioutdir, f'sub-{sub}', 'ecc.nii.gz'),
...
def run_neuropythy(
    ...
    for inname, outname in zip(['pa', 'rsquared', 'ecc', 'sigma'],
                               ['prf_angle', 'prf_vexpl', 'prf_eccen', 'prf_radius']):
        call_vol2surf(sub, afniresults[inname], outname, wdir)
    # run neuropythy
    subprocess.run(
        ['python', '-m', 'neuropythy', 'register_retinotopy', f'sub-{sub}', '--verbose',
    ...
    for param in ['angle', 'eccen', 'sigma', 'varea']:
        subprocess.run(['mri_convert', f'inferred_{param}.mgz', pjoin(outdir, f'inferred_{param}_fsorient.nii.gz')])
        subprocess.run(['fslreorient2std', pjoin(outdir, f'inferred_{param}_fsorient.nii.gz'),
    ...
    for roival in range(1, 13):
        ...
        lres_img.to_filename(pjoin(outdir, f'resampled_va-{roival}_interp-linear.nii.gz'))

    # resample all other outputs too
    for param in ['angle', 'eccen', 'sigma', 'varea']:
        interp = 'nearest' if param == 'varea' else 'linear'
        res_img = resample_to_img(pjoin(outdir, f'inferred_{param}.nii.gz'), ref_img, interpolation=interp)
