...
from os import pardir, getcwd, chmod
...
def grabanat(bidsroot, subject):
    ...
    t1files = thingsmri.layout.get(subject=subject, return_type='file', extension='.nii.gz', suffix='T1w',
                                   reconstruction='pydeface', acquisition='prescannormalized')
    t2file = thingsmri.layout.get(subject=subject, return_type='file', extension='.nii.gz', suffix='T2w',
                                  reconstruction='pydeface', acquisition='prescannormalized')[0]
    return t1files, t2file


def make_reconall_wf(
        subject,
        wdir,
        bidsroot,
        directive='all',
    ...
    wf = Workflow(name='reconall_wf',
                  base_dir=wdir)
    datagrabber = Node(
        Function(function=grabanat,
    ...
    reconall = Node(ReconAll(use_T2=True, directive=directive, openmp=nprocs), name='reconall')
    wf.connect([
